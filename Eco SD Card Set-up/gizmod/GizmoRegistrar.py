...
class GizmoRegistrar:
	...
	def handleDeviceAddition(self):
		...
		if self.DeviceType == DeviceType.Keyboard:
			...
			Gizmod.Standards.append(self.Device)
	...
	def handleDeviceRemoval(self):
		...
		if self.DeviceType == DeviceType.Keyboard:
			...
			for item in Gizmod.Standards:
				if item.FileName == self.Device.FileName:
					Gizmod.Standards.remove(item)
	...
	def __checkDeviceType(self, DeviceTypes):
		...
		for i in DeviceTypes:
			...
			if i[0] == "!":
				NegativeMatch = True
				i = i[1 : len(i)]
			if self.Device.DeviceName.lower().find(i.lower()) > -1:
				if NegativeMatch:
					return False
				else:
					Result = True
			# see if they specified by device node
			elif self.Device.FileName.lower().find(i.lower()) > -1:
				Result = True
			else:
