...
#INTERESTED_WINDOWS = 
INTERESTED_APPLICATION = "REV_HMI_LOTUS"
	...
	def onDeviceEvent(self, Event, Gizmo = None):
		...
		if self.Primary and Event.Type == GizmoEventType.EV_REL:
			if not Gizmo.getKeyState(GizmoKey.BTN_0):
				if Event.Value < 0:				
					#print "Normal rotation left" Shift+Tab
					...
					#print "Normal rotation right" 
					Gizmod.Keyboards[0].createEventPress(GizmoEventType.EV_KEY, GizmoKey.KEY_TAB)
			else:
				if Event.Value < 0:
					#print "Rotation with button eventvalue < 0" Ctrl+RIGHT -> Next tab of tab widget
					...
					#print "Rotation with button eventvalue > 0" Ctrl+LEFT -> Prev tab of tab widget
					Gizmod.Keyboards[0].createEventRaw(GizmoEventType.EV_KEY, GizmoKey.KEY_LEFTCTRL, 1)
