...
void MainWindow::setupDemo(int demoIndex)
{
  switch (demoIndex)
  {
    ...
    case 19: REVgraph(ui->customPlot); break;
  }
...
void MainWindow::setupSimpleDemo(QCustomPlot *customPlot)
{
  ...
  {
    x[i] = i;
    y0[i] = 2*i;//exp(-i/150.0)*cos(i/10.0); // exponentially decaying cosine
    y1[i] = 0.5*i;//exp(-i/150.0);             // exponential envelope
  }
...
void MainWindow::setupRealtimeDataDemo(QCustomPlot *customPlot)
{
  ...
  */
  ...
  // setup a timer that repeatedly calls MainWindow::realtimeTimerSlot:
...
void MainWindow::REVgraph(QCustomPlot *customPlot) {
    customPlot->legend->setVisible(true);
    ...
    while(!in.atEnd()) {
        ...
        ui->customPlot->graph(0)->addData(key, temp.at(0).toInt());
        ui->customPlot->graph(1)->addData(key, temp.at(1).toInt());
