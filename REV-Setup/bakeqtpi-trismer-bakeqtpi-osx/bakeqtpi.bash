...
#CROSSCOMPILETOOLS=$OPT_DIRECTORY/triscross-compile-tools
...
if [[ "$OSTYPE" =~ darwin.* ]]
then
CROSSCOMPILEGIT="gitorious.org/cross-compile-tools/trismers-cross-compile-tools-osx.git"
else
...
RASPBIAN_IMG=$OPT_DIRECTORY/NewPackaged.img
...
if [[ "$OSTYPE" =~ darwin.* ]]
then
wget $WGET_OPT https://docs.google.com/uc?export=download&confirm=bf9E&id=0B_dN9wMSLz89VDZ4SnRiRGRKVkk || error 4
echo "Downloaded toolchain"
tar -xf arm-linux-gnueabihf-osx-2012-08-28.tar || error 5
...
CONFIGURE_OPTIONS="-opengl es2 -device linux-rasp-pi-g++ -device-option CROSS_COMPILE=$CROSSCOMPILER/bin/arm-linux-gnueabihf- -sysroot $ROOTFS -opensource -confirm-license -optimized-qmake -release -make libs -prefix $QT5PIPREFIX -hostprefix $QT5PIHOSTPREFIX -qt-pcre"
...
./configure $CONFIGURE_OPTIONS -v &> results.txt && touch $OPT_DIRECTORY/qt5/qtbase/.CONFIGURED || error 9
#	fi
echo "Making QT Base"
